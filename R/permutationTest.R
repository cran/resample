                             ...
                             block.size = 100, trace = FALSE)
{
  ...
  result <- resample(data, resampleFun, sampler = sampler, B = B,
                     seed = seed,
...
if(FALSE) {
  ...
  permutationTest(x9, mystat)

  # data expression
  permutationTest((x9), mystat)

  # inline function
  permutationTest(x9, function(z) mystat(z))

  # data frame,
  permutationTest(xDF, mycor)

  # data expression, data frame
  permutationTest((xDF), mycor)

  # data expression, matrix
  permutationTest(as.matrix(xDF), mycor)


  ### statistic expression
  # data by name
  permutationTest(x9, mystat(x9))

  # data as expression, refer to 'data'
  permutationTest((x9), mystat(data))

  # data frame
  permutationTest(xDF, mystat(a))
  ...
  # resampleColumns
  permutationTest(xDF, cor(a, b), resampleColumns = "a")
